        ...
        switch (action == null ? "all" : action) {
            ...
            case "update":
                final Meal meal = "create".equals(action) ?
                        new Meal(profileCtrl.get().getId(), LocalDateTime.now().truncatedTo(ChronoUnit.MINUTES), "", 1000) :
                        mealCtrl.get(getId(request));
                request.setAttribute("meal", meal);
                request.getRequestDispatcher("/mealForm.jsp").forward(request, response);
                break;
            case "filter":
                List<MealWithExceed> mealsFiltered = mealCtrl.getFiltered(request.getParameter("startDate"), request.getParameter("endDate"),
            ...
            default:
                log.info("getAll");
                request.setAttribute("meals", mealCtrl.getAll());
                request.getRequestDispatcher("/meals.jsp").forward(request, response);
                break;
        }
    }
